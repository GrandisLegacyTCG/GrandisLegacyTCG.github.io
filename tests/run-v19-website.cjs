...
assert.ok(read('pvp/index.html').includes('gl-pvp-3.17'), 'Embedded public PvP package is not v3.09.');
...
for (const name of fs.readdirSync(path.join(root, 'pvp/starter_deck_examples')).filter(file => file.endsWith('.json'))) {
  ...
  assert.ok(content.includes('Public Deck Builder v1.17'), `${name}: current Deck Builder format metadata is missing.`);
...
console.log('PASS Website v1.17: 198 canonical physical artworks, all 30 revised cards, Back Slash, renamed audio, multilingual Arvon, Rulebook v2, v1.7 homepage layout, PvP v3.17 embed, and source lock.');
