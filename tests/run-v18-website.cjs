...
assert.strictEqual(meta.website_version, '1.8');
...
assert.ok(rulebook.includes('hero-components.js?v=1.0.0') && rulebook.includes('arvon-core.js?v=1.8.0') && rulebook.includes('card-index.js?v=0.13.1'));
assert.ok(read('pvp/index.html').includes('gl-pvp-3.07'), 'Embedded public PvP package is not v3.07.');
...
const lock = json('sync/website-source-lock.v1.8.json');
...
console.log('PASS Website v1.8: v1.7 homepage layout preserved, exact Rulebook PDFs, cards v0.13.1, 30 revisions, Back Slash, Hero Components, multilingual Arvon, PvP v3.07 embed, and source lock.');
