/* Grandis Legacy PvP v3.44 lobby/network integration controller.
   Candidate 2 source/presentation architecture is preserved. Gameplay is intent-only on the client:
   server validation/canonical runtime commits authoritative state, then viewer-safe snapshots cross the
   PvP presentation adapter before the shared VS AI v6.42 Candidate 15 renderer. */
(function(){
  'use strict';
  var VERSION='Grandis Legacy PvP v3.44 · Lobby Restoration + Network Connection Repair · OSA v1.9.5 · 2 Players + 4 Spectators';
...
`;document.head.appendChild(style);}
  ...
  function installLobbyModal(){var wrap=document.createElement('div');wrap.id='pvpSetupOverlay';wrap.className='pvp-v260-lobby';wrap.innerHTML='<div class="pvp-v260-page"><header class="pvp-v260-topbar"><a id="pvpHomeLogo" class="pvp-v260-logo" aria-label="Back to Grandis Legacy home"><img src="assets/lobby/grandis-legacy-logo.webp" alt="Grandis Legacy"></a><div class="pvp-v260-heading"><h1>PVP LOBBY</h1><p>Choose your deck, then ready up or spectate the room.</p></div><nav class="pvp-v260-top-actions"><a id="pvpDeckBuilderLink" class="pvp-v260-btn pvp-v260-outline">GO TO DECK BUILDER</a><a id="pvpAiLobbyLink" class="pvp-v260-btn pvp-v260-blue">GO TO VS AI</a></nav></header><main class="pvp-v260-layout"><section class="pvp-v260-panel pvp-v260-deck-panel"><div class="pvp-v260-picker"><label for="pvpSetupDeck">Choose a Deck</label><div class="pvp-v260-select"><select id="pvpSetupDeck"></select></div></div><div class="pvp-v260-title"><h2 id="pvpDeckTitle">Starter Deck</h2><p id="pvpDeckClassLine"></p></div><div class="pvp-v260-rank-preview" aria-label="Hero Rank Preview"><button id="pvpRankPrev" type="button" aria-label="Previous Hero Rank">←</button><strong id="pvpRankLabel">RANK I</strong><button id="pvpRankNext" type="button" aria-label="Next Hero Rank">→</button></div><div class="pvp-v260-showcase"><div id="pvpFormationPreview" class="pvp-v260-formation"></div><aside class="pvp-v260-summary"><h3>YOUR DECK</h3><dl id="pvpDeckStats"></dl></aside></div><div class="pvp-v260-footer"><input id="pvpImportDeckInput" type="file" accept="application/json,.json" hidden><button id="pvpImportDeckButton" class="pvp-v260-btn pvp-v260-gold pvp-v260-compact" type="button">IMPORT CUSTOM DECK</button><div id="pvpLoadedDeckStatus" class="pvp-v260-status"></div></div></section><aside class="pvp-v260-panel pvp-v260-room-panel"><h2>ROOM PANEL</h2><label class="pvp-v260-label" for="pvpSetupName">PLAYER NAME</label><div class="pvp-v260-name"><input id="pvpSetupName" maxlength="48" placeholder="Your player name" autocomplete="off"><span id="pvpNameStateIcon" class="pvp-v260-name-icon"></span></div><button id="pvpChangeNameButton" class="pvp-v260-btn pvp-v260-outline pvp-name-change" type="button">CHANGE NAME</button><div class="pvp-v260-actions"><button id="pvpSetupSpectatorButton" class="pvp-v260-btn pvp-v260-outline pvp-v260-gold-outline" type="button">SPECTATE</button><button id="pvpSetupReadyButton" class="pvp-v260-btn pvp-v260-gold" type="button">READY</button></div><div class="pvp-v260-divider"></div><div class="pvp-v260-room-id"><div class="pvp-v260-room-copy"><span>CURRENT ROOM</span><strong id="pvpRoomName"></strong></div><button id="pvpSetupSwitchRoom" class="pvp-v260-btn pvp-v260-outline pvp-v260-gold-outline pvp-v260-switch" type="button"></button></div><dl id="pvpSetupStats" class="pvp-v260-room-stats"></dl><div id="pvpSetupPeople" class="pvp-v260-seats"></div><div id="pvpSetupHint" class="pvp-v260-message"></div><div class="pvp-v260-room-footer"><button id="pvpSetupStartButton" class="pvp-v260-btn pvp-v260-gold" type="button" disabled>START MATCH</button><button id="pvpSetupReconnectButton" class="pvp-v260-btn pvp-v260-outline pvp-v260-gold-outline" type="button">RECONNECT</button></div></aside></main></div>';document.body.appendChild(wrap);installProgressionModal();wrap.addEventListener('click',function(ev){var btn=ev.target&&ev.target.closest&&ev.target.closest('[data-pvp-progression]');if(btn){ev.preventDefault();openHeroProgression(btn.getAttribute('data-pvp-progression'));}});
    ...
    $('pvpSetupDeck').addEventListener('change',function(){state.deckKey=this.value||DECK_OPTIONS[0].key;state.lobbyRankPreview=1;loadStarterDeck();});
    $('pvpRankPrev').onclick=function(){setLobbyRankPreview((Number(state.lobbyRankPreview)||1)-1);};$('pvpRankNext').onclick=function(){setLobbyRankPreview((Number(state.lobbyRankPreview)||1)+1);};
  ...
  function loadStarterDeck(){var sel=$('pvpSetupDeck'),key=sel&&sel.value||state.deckKey||DECK_OPTIONS[0].key;if(!key)return false;state.lobbyRankPreview=1;state.deckKey=key;state.loadedDeckKey=key;state.customDeck=null;state.customDeckName='';localStorage.setItem(DECK_KEY,key);localStorage.setItem(LOADED_DECK_KEY,key);if(state.connected&&localRole()!=='spectator')send('set-deck',{deckKey:key});renderPanel();return true;}
  function importCustomDeck(ev){var file=ev&&ev.target&&ev.target.files&&ev.target.files[0];if(!file)return;var reader=new FileReader();reader.onload=function(){try{var parsed=JSON.parse(String(reader.result||''));if(!parsed||!Array.isArray(parsed.main_deck)||!parsed.default_formation)throw new Error('Incomplete deck');var mainCount=parsed.main_deck.reduce(function(sum,entry){return sum+Math.max(0,Math.floor(Number(entry&&entry.quantity!=null?entry.quantity:1)||0));},0);if(mainCount!==60)throw new Error('Main Deck must contain exactly 60 cards.');var copyCounts={};parsed.main_deck.forEach(function(entry){var id=String(entry&& (entry.card_id||entry.cardId||entry.id)||'');var q=Math.max(0,Math.floor(Number(entry&&entry.quantity!=null?entry.quantity:1)||0));if(id&&q)copyCounts[id]=(copyCounts[id]||0)+q;});var runtimeCards=(window.GRANDIS_LEGACY_RUNTIME_DATA&&Array.isArray(window.GRANDIS_LEGACY_RUNTIME_DATA.cards))?window.GRANDIS_LEGACY_RUNTIME_DATA.cards:[];Object.keys(copyCounts).forEach(function(id){var c=runtimeCards.find(function(card){return card&&String(card.card_id||'')===id;})||null;var isUltimate=!!(c&&c.is_ultimate===true);var limit=isUltimate?1:3;if(copyCounts[id]>limit)throw new Error((isUltimate?'Ultimate':'Normal card')+' maximum is '+limit+' cop'+(limit===1?'y':'ies')+': '+id+' x'+copyCounts[id]+'.');});var name=safeName(parsed.display_name||parsed.deck_name||file.name.replace(/\.json$/i,''));state.lobbyRankPreview=1;state.customDeck=parsed;state.customDeckName=name||'Imported Deck';state.loadedDeckKey='CUSTOM';state.deckKey='';localStorage.removeItem(LOADED_DECK_KEY);if(state.connected&&localRole()!=='spectator')send('set-deck',{customDeck:parsed,deckName:state.customDeckName});setStatus('online','Custom deck loaded: '+state.customDeckName);renderPanel();}catch(err){var reason=String(err&&err.message||'Invalid custom deck JSON.');setStatus('offline',reason);var msg=$('pvpSetupHint');if(msg)msg.textContent=reason;}};reader.readAsText(file);if(ev.target)ev.target.value='';}
  ...
  function connect(force){
    ...
    ws.onopen=function(){clearConnectTimeout();clearIntentLock();state.connected=true;state.latencyMs=null;state.lastPongAt=Date.now();syncConnectionSignalGlobals();setTimeout(syncBattlefieldIdentityHeaders,0);reconnectDelay=1200;setConnectionState('connected','Connected to '+roomDisplayName(),target);renderPanel();if(state.name)send('rename',{name:state.name});if(state.role==='player'&&state.customDeck)send('set-deck',{customDeck:state.customDeck,deckName:state.customDeckName});else if(state.role==='player'&&activeLoadedDeckKey())send('set-deck',{deckKey:activeLoadedDeckKey()});};
    ...
    ws.onmessage=function(ev){var msg;try{msg=JSON.parse(ev.data);}catch(e){return;}if(msg.type==='pong'){noteServerBuild(msg);var now=Date.now(),sent=Number(msg.clientAt||state.lastPingSentAt||0);if(sent>0&&now>=sent)state.latencyMs=Math.max(0,now-sent);state.opponentLatencyMs=(msg.opponentLatencyMs!==null&&msg.opponentLatencyMs!==undefined&&Number.isFinite(Number(msg.opponentLatencyMs)))?Math.max(0,Math.round(Number(msg.opponentLatencyMs))):null;state.lastPongAt=now;state.lastPingSentAt=0;syncConnectionSignalGlobals();syncBattlefieldIdentityHeaders();return;}if(msg.type==='snapshot'){handleSnapshot(msg);return;}if(msg.type==='intent-ack'){if(state.intentInFlight&&(!state.intentActionId||!msg.clientActionId||msg.clientActionId===state.intentActionId))setStatus('connecting','Server received '+(msg.intent||state.intentName||'action')+(msg.duplicate?' · duplicate safely ignored':' · resolving...'));return;}if(msg.type==='idle-warning'){showIdleWarning(msg);return;}if(msg.type==='idle-released'){closeIdleWarning();setStatus('offline',msg.message||'Player seat released due to inactivity.');return;}if(msg.type==='notice'){if(msg.kind==='error')clearIntentLock();setStatus(msg.kind==='error'?'offline':'online',msg.message||'Notice');return;}if(msg.type==='fatal'){setConnectionState('error',msg.message||'Fatal room error',target);try{ws.close();}catch(e){}}};
  }
